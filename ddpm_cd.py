...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/cdd.json',
    ...
    if opt['phase'] == 'train':
        for current_epoch in range(start_epoch, n_epoch):         
            ...
            prog_bar = tqdm(train_loader, desc="Epoch {}".format(current_epoch))

            for current_step, train_data in enumerate(prog_bar):
                ...
                change_detection.feed_data(f_A, train_data)
                ...
                if current_step % opt['train']['train_print_freq'] == 0:
                    ...
                    # if img_mode == "single":
                    #     # Converting to uint8
                    #     img_A   = Metrics.tensor2img(train_data['img'][:,0:1,:,:], out_type=np.uint8, min_max=(-1, 1))  # uint8
                    ...
                    #     grid_img = torch.cat((  train_data['img'][:,0:1,:,:],
                    #                 visuals['pred_cm'].unsqueeze(1).repeat(1, 1, 1, 1),
                    #                 visuals['gt_cm'].unsqueeze(1).repeat(1, 1, 1, 1)),
                    #                 dim = 0)
            ...
            if current_epoch % opt['train']['val_freq'] == 0:
                ...
                prog_valbar = tqdm(val_loader, desc="Validation {}".format(current_epoch))

                for current_step, val_data in enumerate(prog_valbar):
                    ...
                    change_detection.feed_data(f_A, val_data)
                    change_detection.test()
                    change_detection._collect_running_batch_states()

                    IoU_sum += change_detection.get_current_results()
                ...
                message = '[Validation CD]. epoch: [%d/%d], mIoU: %.5f\n'%(current_epoch, n_epoch-1,  mIoU)
        ...
        for current_step, test_data in enumerate(test_loader):
            ...
            # Feed data to CD model
            change_detection.feed_data(f_A,  test_data)
            change_detection.test()
            change_detection._collect_running_batch_states()
            ...
            if img_mode == 'single':
                # Converting to uint8
                img_num = test_data['img_num'].detach().cpu().numpy().astype(int)
                # visuals['pred_cm'] = visuals['pred_cm']*2.0-1.0
                # visuals['gt_cm'] = visuals['gt_cm']*2.0-1.0
                img_A   = Metrics.tensor2img(test_data['img'][:,0:1,:,:], out_type=np.uint8, min_max=(-1, 1))  # uint8
                # img_B   = Metrics.tensor2img(test_data['B'], out_type=np.uint8, min_max=(-1, 1))  # uint8
                # gt_cm   = Metrics.tensor2img(visuals['gt_cm'].unsqueeze(1).repeat(1, 1, 1, 1), out_type=np.uint8, min_max=(0, 1))  # uint8
                pred_cm = Metrics.tensor2img(visuals['pred_cm'].unsqueeze(1).repeat(1, 1, 1, 1), out_type=np.uint8, min_max=(0, 1))  # uint8
                ...
                Metrics.save_img(
                    pred_cm, '{}/{}.png'.format(test_result_path, str(img_num.item()).zfill(4)))
                # Metrics.save_img(
                ...
                visuals['pred_cm'] = visuals['pred_cm']*2.0-1.0
                visuals['gt_cm'] = visuals['gt_cm']*2.0-1.0
                grid_img = torch.cat((  test_data['img'][:,0:1,:,:],
                                        visuals['gt_cm'].unsqueeze(1).repeat(1, 1, 1, 1),
                                          visuals['pred_cm'].unsqueeze(1).repeat(1, 1, 1, 1) ),
                                        dim = 0)
                grid_img = Metrics.tensor2img(grid_img)  # uint8
                ...
                img_num = test_data['img_num'].detach().cpu().numpy().astype(int)
                Metrics.save_img(
                    grid_img, '{}/img_A_B_pred_gt_{}.png'.format(test_result_path, str(img_num.item()).zfill(4)))

        change_detection._collect_epoch_states()

        logs = change_detection.get_current_log()

        message = '[Test CD summary]: Test mIoU=%.5f \n' %\
                      (IoU_sum/len(test_loader))
