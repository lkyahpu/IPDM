...
class DDPM(BaseModel):
    ...
    def test(self, input, in_channels, img_size, continous=False):
        self.netG.eval()
        with torch.no_grad():
            input = self.set_device(input)
            if isinstance(self.netG, nn.DataParallel):
                self.sampled_img = self.netG.module.sampling_imgs(input,
                    in_channels, img_size, continous)
            else:
                self.sampled_img = self.netG.sampling_imgs(input,
                    in_channels, img_size, continous)
        self.netG.train()
    ...
    def get_feats(self, t):
        ...
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                ...
                fe_B, fd_B = self.netG.module.feats(self.data, t)
            else:
                fe_A, fd_A = self.netG.feats(self.data, t)
                # fe_B, fd_B = self.netG.feats(self.data, t)
        self.netG.train()
