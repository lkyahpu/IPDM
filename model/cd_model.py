...
from .metrics import metrics, ROCMetric, PRMetric, IoU
import pytorch_iou
...
class CD(BaseModel):
    def __init__(self, opt):
        ...
        self.IOU_loss = pytorch_iou.IOU(size_average=True).to(self.device)
    ...
    def feed_data(self, feats_A, data):
    ...
    def optimize_parameters(self):
        ...
        self.pred_cm = self.netCD(self.feats_A)
        # pred_d = torch.argmax(self.pred_cm, dim=1, keepdim=False)

        # l_cd = self.loss_func(self.pred_cm, self.data["label"].long()) +  self.IOU_loss(
        #     torch.argmax(self.pred_cm, dim=1, keepdim=False), self.data["label"]) + \
        #        0.5*get_saliency_smoothness(torch.argmax(self.pred_cm, dim=1, keepdim=False).float().unsqueeze(1), self.data["label"].float().unsqueeze(1))
        l_cd = self.loss_func(self.pred_cm, self.data["label"].long()) + 2*self.IOU_loss(
                torch.argmax(self.pred_cm, dim=1, keepdim=False), self.data["label"])
    ...
    def test(self):
        ...
        with torch.no_grad():
            if isinstance(self.netCD, nn.DataParallel):
                self.pred_cm = self.netCD.module.forward(self.feats_A)
            else:
                self.pred_cm = self.netCD(self.feats_A)
    ...
    def get_current_visuals(self):
        ...
        # pre = F.interpolate(self.pred_cm, size=(512, 640), mode='bilinear', align_corners=True)
        # gt = F.interpolate(self.data['label'].unsqueeze(1), size=(512, 640), mode='bilinear', align_corners=True)

        pre = torch.argmax(self.pred_cm, dim=1, keepdim=False)  #self.pred_cm[:,1,:,:]
        out_dict['pred_cm'] = pre  # torch.argmax(pre, dim=1, keepdim=False)
        out_dict['gt_cm'] = self.data['label']
        return out_dict
    ...
    def get_current_results(self):
        ...
        # pre = F.interpolate(self.pred_cm, size=(512, 640), mode='bilinear', align_corners=True)
        # out_dict['pred_cm'] = torch.argmax(pre, dim=1, keepdim=False)
        # out_dict['gt_cm'] = self.data['label']

        # pre = torch.argmax(self.pred_cm, dim=1, keepdim=False)
        gt = self.data['label']
        gt = gt.squeeze().detach().cpu().numpy().astype('uint8')

        # pre = F.interpolate(self.pred_cm, size=(512, 640), mode='bilinear', align_corners=True)
        # pre = self.pred_cm
        # pre = np.squeeze(pre.cpu().data.numpy())
        # pre = (pre - pre.min()) / (pre.max() - pre.min() + 1e-8)
        pre = torch.argmax(self.pred_cm, dim=1, keepdim=False)
        pre = pre.squeeze().detach().cpu().numpy().astype('uint8')
        # pre = (pre - pre.min()) / (pre.max() - pre.min() + 1e-8)
    ...
    def _update_metric(self):
        ...
        current_score = self.running_metric.update_cm(pr=G_pred.cpu().numpy(),
                                                      gt=self.data['label'].detach().cpu().numpy())
